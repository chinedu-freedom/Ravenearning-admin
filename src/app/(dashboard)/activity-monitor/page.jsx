...
const formatDetails = (details, action, userAgent) => {
  ...
  try {
    ...
    if (actionKey.includes("package purchase") || actionKey.includes("investment")) {
      return `${parsed.planName || "Plan"} (R${parsed.amount || 0})`;
    }
    if (actionKey.includes("daily check in") || actionKey.includes("checkin")) {
      return `Day ${parsed.day || "-"} reward (R${parsed.amount || 0})`;
    }
    if (actionKey.includes("bonus claimed")) {
      return `Code: ${parsed.code || "-"} (R${parsed.amount || 0})`;
    }
    if (actionKey.includes("spin wheel") || actionKey.includes("spin")) {
      return `Won ${parsed.prizeName || "Reward"} (R${parsed.rewardAmount || 0})`;
    }
    if (actionKey.includes("deposit initiated")) {
      return `Requested R${parsed.amount || 0} (R{parsed.cryptocurrency || "Crypto"})`;
    }
    if (actionKey.includes("deposit completed")) {
      return `Credited R${parsed.amount || 0}`;
    }
    if (actionKey.includes("withdrawal requested")) {
      return `Requested R${parsed.amount || 0} (R{parsed.network || "Crypto"})`;
    }
    if (actionKey.includes("admin credit")) {
      return `${parsed.balance_type === "gift" ? "Gift" : "Main"} Credit: R${parsed.amount || 0} (R{parsed.reason || "No reason"})`;
    }
    if (actionKey.includes("admin debit")) {
      return `${parsed.balance_type === "gift" ? "Gift" : "Main"} Debit: R${parsed.amount || 0} (R{parsed.reason || "No reason"})`;
    }
